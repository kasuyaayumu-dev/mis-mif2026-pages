...
- `data/` — マーカー・タイムテーブルデータ(JSON、手動更新)
  - `groups.json` — 2026 MIFデザイン部情報フォームの回答を元にした企画・団体情報一覧(63件)。企画名・団体名・短い概要(日英)・カテゴリ・形式など。Map等では短い概要のみ使用するため、長い説明文(企画説明)は含めていない。`floor`/`room`/`latlng`はマップ座標未確定のため`null`のプレースホルダーで、確定次第そのまま埋めればマップ用データとしても使える。
